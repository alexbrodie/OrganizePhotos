...
sub trash_path_with_root {
    ...
    my @main_dir_parts = File::Spec->splitdir(
        File::Spec->catdir( $main_dir, $main_filename ) );

    # Example N: root_dir_parts = ( ..., root )
    my @root_dir_parts = File::Spec->splitdir(
        File::Spec->catdir( $root_dir, $root_filename ) );
