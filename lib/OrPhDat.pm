...
sub resolve_orphdat {
    ...
    unless ($force_recalc) {
        if ( defined $cached_orphdat ) {
            my $cache_result = check_cached_orphdat( $path, $add_only, 'Caller',
        ...
        sub {        # callback
            ...
            if ( -f $path ) {
                    ...
                    )
                {
                    ...
                    if (
                        $is_file_wanted->(
                            $other_path, $root, $other_filename
                        )
                        )
                    {
...
sub check_cached_orphdat {
    ...
    if ($add_only) {
        ...
        unless (
            lc $current_orphdat_base->{filename} eq
            lc $cached_orphdat->{filename} )
        {
            push @delta, 'name';
        }
        unless ( defined $cached_orphdat->{size}
            and $current_orphdat_base->{size} == $cached_orphdat->{size} )
        {
            push @delta, 'size';
        }
        unless ( defined $cached_orphdat->{mtime}
            and $current_orphdat_base->{mtime} == $cached_orphdat->{mtime} )
        {
            push @delta, 'mtime';
        }
        if (@delta) {
